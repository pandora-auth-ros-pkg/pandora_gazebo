...
#include "pandora_gazebo_plugins/pandora_imu_stabilizer_plugin.h"
...
void GazeboRosIMU::LoadThread()
{
  ...
  if (!this->sdf->HasElement("rollCorrectionJoint"))
  {
    ROS_FATAL("stabilizer plugin missing <rollCorrectionJoint>, exiting");
    return;
  }
  else
  {
